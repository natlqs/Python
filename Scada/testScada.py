...
stat = client.connect('192.168.0.2', 0, 2)
...
def updt():
    t1 = time.time()
    DB[110] = client.db_read(110, 0, sizeDB)                            # 必須要求對DB進行異步讀取
    DB[104] = client.db_read(104, 0, sizeDB)
    #print(get_int(DB[104],274))
    t2 = time.time()
    for gameObj in gameObjects:
                gameObj.update()
    c1.itemconfigure(tm, text = (round(t2-t1,4)), fill="red")            # 必須要求對DB進行異步讀取
    root.after(500, updt)
...
menu = Menu(tearoff=0)
menu.add_command(label="Editing",command=test1)
menu.add_command(label="Stop Update",command=test1)
menu.add_command(label="Exit Program",command=test1)
