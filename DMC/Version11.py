...
file_choose = tk.LabelFrame(
    GUI, text='源文件夹选择', font='宋体 15 bold', padx=10, pady=10)
...
TimeLabel = tk.Label(text="%s" % (datetime.datetime.now().strftime(
    '%Y-%m-%d %H:%M:%S'), ), font='宋体 26 bold')
TimeLabel.place(x=330, y=85)


compare_result_frame = tk.LabelFrame(
    GUI, text='检查结果',  font='宋体 20 bold', padx=10, pady=10)
compare_result_frame.place(x=320, y=125)
compare_result_text = scrolledtext.ScrolledText(
    compare_result_frame, bg='white', font='宋体 15 bold', width=30, height=10, padx=10, pady=10)
compare_result_text.grid()
compare_result_text.insert('end', '\n' + '\n' + '时间： ')
compare_result_text.insert(
...
def Compare_Result():
    this_week_csv = file_name[0] + '\\' + time.strftime(
        '%Y%m%d %H:%M:%S', time.localtime(get_week_begin(time.time(), 0)))[2:8] + '.csv'
    try:
        with open(this_week_csv, 'r') as read_data:
            ...
            if g_value0 == 'ee':
                ...
                compare_result_text.insert('end', '请确认该产品不是上周库存件，且是1#动平衡生产的产品')
                compare_result_text.insert('end', '数据上次更新时间：' + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(
                    os.stat(this_week_csv).st_mtime)) + '。 如生产中且超过5分钟数据未更新，请检查并重启数据保存软件')
                compare_result_text.insert('end', '\n' + '请扫描下一个产品二维码')
            else:
        ...
        compare_result_text.insert('end', '\n' + '文件未找到，请检查源文件路径是否正确' + '\n')
        compare_result_text.config(bg='red')
        compare_result_text.insert('end', '\n' + '请检查源文件' + time.strftime('%Y%m%d %H:%M:%S',
                                   time.localtime(get_week_begin(time.time(), 0)))[2:8] + '.csv' + '是否存在' + '\n')
        pass
...
tk.Button(switch1, text="打开端口", font='宋体 18 bold',
          command=Open_Serial).pack(side="left", padx=2)
tk.Button(switch1, text='开始检查', font='宋体 18 bold',
          command=Start_Compare).pack(side='left', padx=2)
tk.Button(switch1, text="关闭端口", font='宋体 18 bold',
          command=Close_Serial).pack(side="left", padx=2)
tk.Button(file_choose, text='选择源文件', font='宋体 15 bold',
          command=File_Choose_Button).grid(row=1, column=0, pady=2)
