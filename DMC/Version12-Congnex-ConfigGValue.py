...
# Version 10: 添加时间实时显示功能
# Version 11: 添加显示文件数据更新时间,并提示检查是否库存件
# Version 12: 可自定义G值
#! /usr/bin/env python
# -*- coding: utf-8 -*-
...
GUI.geometry("720x480")  # 主窗口大小
GUI.wm_attributes('-topmost', 1)

Receive = tk.LabelFrame( GUI, text = "端口状态", font='宋体 15 bold', padx = 10, pady = 10 )  # 水平，垂直方向上的边距均为 10
Receive.place(x = 5, y = 175)
Receive_Window = scrolledtext.ScrolledText(Receive,font='宋体 12 bold', width = 28, height = 4, padx = 8, pady = 10,wrap = tk.WORD)
Receive_Window.grid()
 
G_Value_Frame = tk.LabelFrame( GUI, text = "G值设定", font='宋体 15 bold', padx = 10, pady = 10 )  # 水平，垂直方向上的边距均为 10
G_Value_Frame.place(x = 5, y = 105, width=310)
ttk.Label( G_Value_Frame, text = "G值设定:" ,font='宋体 15 bold').grid( column = 0, row = 1 )  # 添加G值设定标签
G_Value_String = tk.StringVar()  # G值设定字符串
G_Value_String_List = ttk.Combobox( G_Value_Frame, width = 15,font='宋体 15 bold', textvariable = G_Value_Frame, state = 'readonly' )
G_Value_String_List['values'] = (7000, 8000, 9000, 10000, 11000, 12000, 13000, 14000, 15000, 16000, 17000, 18000, 19000, 20000)
G_Value_String_List.current(3)
G_Value_String_List.grid(column=1, row=1)  # 设置其在界面中出现的位置  column代表列   row 代表行
...
file_choose.place(x=5, y=310, width=310)
...
#TimeLabel = tk.Label(text = "%s" %(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), ), font='宋体 26 bold', background='#FE8C00')
TimeLabel = tk.Label(text = "%s" %(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), ), font='宋体 26 bold')
TimeLabel.place(x = 330, y = 85)


compare_result_frame=tk.LabelFrame(GUI, text='检查结果',  font='宋体 20 bold', padx=10, pady=10)
compare_result_frame.place(x=320, y=130)
    ...
    try:
        with open(this_week_csv, 'r') as read_data:
            ...
            if g_value0=='ee':
                ...
                compare_result_text.insert('end', '请确认该产品不是上周库存件，且是目标动平衡生产的产品')
                compare_result_text.insert('end', '数据上次更新时间：' + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(os.stat(this_week_csv).st_mtime)) + '。 如生产中且超过5分钟数据未更新，请检查并重启数据保存软件')
                compare_result_text.insert('end', '\n' + '请扫描下一个产品二维码')
            else:
                if float(g_value0) < int(G_Value_String_List.get()):
                    compare_result_text.insert('end', '\n' + 'G 值结果：' + '专用F202'+'\n'*3)
