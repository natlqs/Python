...
file_choose_text_source = scrolledtext.ScrolledText(file_choose, width = 25, height = 3, padx=6, pady = 6, wrap=tk.WORD)
file_choose_text_source.grid(column=0, row=0)
file_choose_text_destination= scrolledtext.ScrolledText(file_choose, width=25, height = 3, padx=6, pady=10, wrap = tk.WORD)
...
def Select_Destination_Folder():
    ...
    file_choose_text_destination.insert('end', '\n' + str(filepath_destination) + '\n')
...
def File_Write():
    ...
    '''
    for i in filenames:
        last_friday = time.strftime('%Y%m%d %H:%M:%S',time.localtime(get_day_begin(get_week_begin(time.time(), 0)-172800,0)))[2:8]       # 上周五        211023 
...
tk.Button(start_switch, text='开始自动保存', font='宋体 13 bold', command= Cycle_Refresh).pack(padx=10) 
tk.Button(file_choose, text='选择源数据文件夹', font='宋体 12 bold', command=Select_Source_Folder).grid(column=1,row=0)
tk.Button(file_choose, text='选择目的文件夹', font='宋体 12 bold', command=Select_Destination_Folder).grid(column=1, row=1)
GUI.mainloop()